...
async function saveImage(image: File | null, id: string, title: string): Promise<string> {
    if (!image || !id) return "";
...
export async function createStartup({
    ...
    try {
        ...
        const imageUrl = await saveImage(imageFile, userId!.toString(), titleString);
...
export async function deleteStartup(id: string) {
    try {
        ...
        const startup = await writeClient.fetch(`*[_type == "startup" && _id == $id][0]`, { id });
        ...
        const playlists = await writeClient.fetch(`*[_type == "playlist" && references($id)]`, {
            id
        });
        ...
        if (imageUrl) {
            try {
                const imagePath = path.join(process.cwd(), "public", new URL(imageUrl).pathname);
