...
// Query: *[_type == "startup" && defined(slug.current) && !defined($search) || category match $search || title match $search] | order(_createdAt desc) {  _id,    title,    slug,    _createdAt,    author -> {      _id, name, email, image, bio    },      views,      description,      category,      image}
...
// Query: *[_type == "startup" && _id == $id][0] {    _id,      title,      slug,      _createdAt,      author -> {        _id, name, email, image, bio, username      },        views,        description,        category,        image,        pitch  }
...
// Query: *[_type == "startup" && author._ref == $id] | order(_createdAt desc) {  _id,    title,    slug,    _createdAt,    author -> {      _id, name, email, image, bio    },      views,      description,      category,      image}
...
declare module "@sanity/client" {
  interface SanityQueries {
    "*[_type == \"startup\" && defined(slug.current) && !defined($search) || category match $search || title match $search] | order(_createdAt desc) {\n  _id,\n    title,\n    slug,\n    _createdAt,\n    author -> {\n      _id, name, email, image, bio\n    },\n      views,\n      description,\n      category,\n      image\n}": STARTUPS_QUERYResult;
