...
import { createStartup, deleteStartup, updateStartup } from "@/app/actions/startup-actions";
...
export const useStartupForm = () => {
    ...
    const handleUpdateStartup = async (formData: FormData, startupId: string, userId?: string) => {
        setFormState((prevState) => ({ ...prevState, status: "SUBMITTING" }));
    ...
    const handleDeleteStartup = async (startupId: string) => {
        ...
        try {
            ...
            toast({
                title: "Error",
                description: error instanceof Error ? error.message : "Failed to delete startup.",
                variant: "destructive"
            });
        } finally {
            setFormState((prevState) => ({
